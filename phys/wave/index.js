...
function randomWaves2(c, pD) {
  for (let i = 0; i < N; i++) {
    for (let j = 0; j < N; j++) {
      const x = fromX + i / N * width;
      const y = fromX + j / N * width;

      const x1 = c1 * Math.cos(x + pD + f2);
      const y1 = c2 * Math.sin(y + pD + f1);

      const x2 = x1 + c3 * Math.cos(pD + f3);
      const y2 = y1 + c4 * Math.sin(pD + f4);

      const x3 = x2 + c5 * Math.cos(pD + f5);
      const y3 = y2 + c6 * Math.sin(pD + f6);
...
function randomWaves(c, pD) {
  for (let i = 0; i < N; i++) {
    for (let j = 0; j < N; j++) {
      ...
      const x1 = x + c1 * Math.cos(f2 * x + pD);
      const y1 = y + c2 * Math.sin(f2 * y + pD);

      const x2 = x1 + c3 * Math.cos(f3 * x1);
      const y2 = y1 + c4 * Math.sin(f3 * y1);

      const x3 = x2 + c5 * Math.cos(f4 * x2);
      const y3 = y2 + c6 * Math.sin(f4 * y2);

      const x4 = x3 + c7 * Math.cos(f5 * x3);
      const y4 = y3 + c8 * Math.sin(f5 * y3);
